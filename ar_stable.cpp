...
bool checkDiffRot(Vec3d rvec1, Vec3d rvec2, std::vector<double> thr) {
    ...
    for(int i=0; i<3; i++) {
        if(std::abs(sin(rvec1_eul[i])-sin(rvec2_eul[i])) > std::abs(sin(thr[i]))) {
            return false;
        }
    }
    return true;
    ...
    for(unsigned int i=0; i<rvecs.size(); i++) {
        ...
        for(unsigned int j=0; j<rvecs.size(); j++) {
            ...
            for(int k=0; k<3; k++) {
                ...
                if(std::abs(sin(rvecs[i][k])-sin(rvecs[j][k])) > std::abs(sin(thr[k]))) {
                    checker[i][j] = false;
                    cout << "False" << endl;
                    break;
                }
...
int main(int argc, char *argv[]) {
    ...
    thr_init[0] = thr_init[1] = thr_init[2] = 60.0;
