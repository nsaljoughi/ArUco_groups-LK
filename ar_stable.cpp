...
int main(int argc, char *argv[]) {
    ...
    vector<Point2d> box, box1, box2, box3, box4, box5, box6, box7, box8; // vec to print arrow on image plane
    ...
    while(inputVideo.grab()) {
		...
		boxes5.push_back(box5);
		boxes6.push_back(box6);
		boxes7.push_back(box7);
		boxes8.push_back(box8);
            }
            else if (scene==1) {
                boxes1.push_back(box1);
                boxes2.push_back(box2);
	    ...
	    if (init_id[0] || init_id[4] || init_id[8]) {
            ...
            if (scene==3) {
                ...
                projectPoints(box_cloud, Vec3d::zeros(), avg_points[4], camMatrix, distCoeffs, box5);
                projectPoints(box_cloud, Vec3d::zeros(), avg_points[5], camMatrix, distCoeffs, box6);
                projectPoints(box_cloud, Vec3d::zeros(), avg_points[6], camMatrix, distCoeffs, box7);
                projectPoints(box_cloud, Vec3d::zeros(), avg_points[7], camMatrix, distCoeffs, box8);
		...
		box5 = avgBoxes(boxes5, weights);
		box6 = avgBoxes(boxes6, weights);
		box7 = avgBoxes(boxes7, weights);
		box8 = avgBoxes(boxes8, weights);
		...
		box3 = avgBoxes(boxes3, weights);
           ...
           if(init_id[0]||init_id[4]||init_id[8]) {
               if (scene==3) {
                   ...
                   DrawBox2D(imageCopy, box2, 60, 20, 220);
                   DrawBox2D(imageCopy, box3, 60, 20, 220);
                   DrawBox2D(imageCopy, box4, 60, 20, 220);
                   DrawBox2D(imageCopy, box5, 60, 20, 220);
                   DrawBox2D(imageCopy, box6, 60, 20, 220);
                   DrawBox2D(imageCopy, box7, 60, 20, 220);
                   DrawBox2D(imageCopy, box8, 60, 20, 220);
               }
               else if (scene==1) {
                   DrawBox2D(imageCopy, box1, 60, 20, 220);
                   DrawBox2D(imageCopy, box2, 60, 20, 220);
